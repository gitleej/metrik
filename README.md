...
3. **统计与报表**：26 周热力图、周趋势折线、Agent 占比环形图、项目走势；用量页上半是项目汇总、下半是会话明细，点项目行即筛选会话，支持 CSV 导出。项目按各 Agent 记录的工作目录归集，默认向上合并到 git 仓库根，也可手动登记项目根或隐藏目录；读不到目录的用量单独标为未归属。完整面板分概览、用量、报告、设置四页，主题可跟随系统或手动指定。
